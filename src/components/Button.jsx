const Button = ({ children, variant = "filled", Icon, ...props }) => {
  ...
  return (
    <button
      className={`dark:text-white text-xs sm:text-sm md:text-md py-3 px-8 rounded-full font-semibold flex items-center gap-2 transition duration-300 cursor-pointer ${styles[variant]}`}
      {...props}
    >
      {children}
      {Icon && Icon}
